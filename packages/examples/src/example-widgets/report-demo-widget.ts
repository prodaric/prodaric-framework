...
async function downloadReportPdf(reportId: string, statusEl: HTMLElement): Promise<void> {
  ...
  try {
    ...
    statusEl.textContent =
      'Servidor no disponible. Ejecute: npm run report-on-demand (asegura el runtime y arranca el servidor en un solo paso)';
...
export function createReportDemoWidget(): Widget {
  ...
  header.innerHTML = `
    ...
    <p>Capacidad del framework: reportes generados bajo demanda. Cada demo (Productos, Resumen, Ventas) genera un PDF al pulsar "Descargar PDF". Requiere <code>npm run report-on-demand</code> y <code>./scripts/report-server.sh ensure</code>. Vista previa de datos en tabla.</p>
  `;
